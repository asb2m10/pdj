...
/**
 * Casts Java Atom[] to pd t_atom. If the stack is not return by this function,
 * it must be freed. Use with care.
 */
int jatoms2atoms(JNIEnv *env, jobjectArray jatoms, int *nb_atoms, t_atom **atoms) {
	jobject obj;
	int i, rc = 0;
    int initial_size = *nb_atoms;
	
	*nb_atoms = (*env)->GetArrayLength(env, jatoms);
	
	if ( *nb_atoms > initial_size ) {
        *atoms = malloc(sizeof(t_atom) * *nb_atoms);
        ASSERT(atoms);    
	}
