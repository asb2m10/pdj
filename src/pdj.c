...
static void pdj_process_inlet(int idx, t_pdj *pdj, t_symbol *s, int argc, t_atom atoms[]){
    ...
    symbol = (*env)->NewStringUTF(env, s->s_name);
	if ( argc == 0 ) {
	    (*env)->CallVoidMethod(env, pdj->obj, pdjCaching.MIDMaxObject_processInlet, symbol, NULL);	    
	} else {
		args = atoms2jatoms(env, argc, atoms);
    	JASSERT(args);
	    (*env)->CallVoidMethod(env, pdj->obj, pdjCaching.MIDMaxObject_processInlet, symbol, args);
    }		
    SHOWEXC;
