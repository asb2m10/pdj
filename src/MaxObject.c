...
JNIEXPORT void JNICALL Java_com_cycling74_max_MaxObject_doOutletAnything
  (JNIEnv *env, jobject obj, jlong outlet, jstring str, jobjectArray value) {
  	t_outlet *x = (t_outlet *) (JPOINTER_CAST outlet);
    t_atom argv_onstack[MAX_ATOMS_STACK];
    t_atom *argv = (t_atom *) &argv_onstack;
  	int argc = MAX_ATOMS_STACK;

    if ( jatoms2atoms(env, value, &argc, &argv) == 0 ) {
  	    if ( str == NULL ) {
  		    if ( argv[0].a_type == A_FLOAT ) {
  			    outlet_anything(x, &s_list, argc, argv);
  		    } else {
	  		    t_symbol *sym = atom_getsymbol(&(argv[0]));
	  		    outlet_anything(x, sym, argc-1, argv+1);
  		    }
  	    } else {
  		    outlet_anything(x, jstring2symbol(env, str), argc, argv);
  	    }
    }
  	
    if ( argc > MAX_ATOMS_STACK )
        free(argv);
}
