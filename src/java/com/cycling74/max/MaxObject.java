...
public class MaxObject {
		...
		try {
			...
			if ( args.length > 0 ) {
				try {
					...
					Constructor c = clz.getConstructor(param);
					Object argValue[] = new Object[args.length];
					for (int i = 0; i < args.length; i++) {
						if (args[i].isString())
							argValue[i] = args[i].toString();
						else
							argValue[i] = new Float(args[i].toFloat());
					}					
                ...
                try {
                    ...
                    try {
                        Constructor c = clz.getConstructor(argType);
                        obj = (MaxObject) c.newInstance((Atom[]) new Atom[1] );
                    } catch ( Exception e1 ) {
                        popPdjPointer();
                        throw e1;
                    }
	...
	Hashtable _inlet_dispatch = new Hashtable();
	private void _processInlet(String symbol, Atom args[]) throws Exception {
	    String signature = symbol;
	    
	    if (args == null ) {
            if ( _inlet_dispatch.containsKey(signature) ) {
                InletDispatch dispatch = (InletDispatch) _inlet_dispatch.get(signature);
                if ( dispatch.method == null ) {
                	anything(symbol, null);
                } else {
                	dispatch.method.invoke(this, null);
                }
                return;
            }
        } else if ( args.length == 1 ) {
            signature = symbol + (args[0].isString() ? "S" : "F");
            if ( _inlet_dispatch.containsKey(signature) ) {
                InletDispatch dispatch = (InletDispatch) _inlet_dispatch.get(signature);
                if ( dispatch.method == null ) {
                	anything(symbol, args);
                	return;
                }
                if ( dispatch.atoms ) {
                	dispatch.method.invoke(this, new Object[] { args } );
                } else {
                	if ( args[0].isString() ) {
                		dispatch.method.invoke(this, new Object[] { args[0].toString() });
                	} else {
                		dispatch.method.invoke(this, new Object[] { new Float(args[0].toFloat()) } );
                	}
                }
                return;
            }
            
            if ( symbol.equals("get") ) {
        		Attribute attr = (Attribute) attributes.get(args[0].toString());
        		if ( attr != null ) {
        			if ( toCreateInfoOutlet )
        				outlet(_outlets_ptr.length-1, attr.get());
            		return;
        	...
        	StringBuffer sigbuilder = new StringBuffer();
        	
        	sigbuilder.append(symbol);
        	for(int i=0;i<args.length;i++)
        		sigbuilder.append(args[i].isString() ? "S" : "F");
            ...
            if ( _inlet_dispatch.containsKey(signature) ) {
				...
				if (dispatch.atoms) {
					...
					for (int i = 0; i < args.length; i++) {
						if (args[i].isString())
							obj[i] = args[i].toString();
						else
							obj[i] = new Float(args[i].toFloat());
					}
					dispatch.method.invoke(this, obj);
				}
				return;
			} else if ( symbol.equals("set") ) {
            	Attribute attr = (Attribute) attributes.get(args[0].toString());
            	if ( attr != null ) {
                	Atom value[] = new Atom[args.length-1];
	    ...
	    /* not in cache, resolv method */
	    InletDispatch dispatch = new InletDispatch();
	    
	    if ( args == null ) {
	    	try {
	    		// try with parent methods
	    		dispatch.method = getClass().getMethod(symbol, new Class[0]);
	    	} catch (NoSuchMethodException e) {
	    		try {
	    			// try the class it self (for package methods)
	    			dispatch.method = getClass().getDeclaredMethod(symbol, new Class[0]);
	    		} catch (NoSuchMethodException e1) {
	    		}
	    	}
	    	...
	    	try {
	    		dispatch.method = getClass().getMethod(symbol, param);
	    		dispatch.atoms = false;
	    	} catch (NoSuchMethodException e) {
	    		try {
	    			dispatch.method = getClass().getDeclaredMethod(symbol, param);
	    			dispatch.atoms = false;
	    		} catch (NoSuchMethodException e2) {	
			    	try {
			    		dispatch.method = getClass().getMethod(symbol, new Class[] { args.getClass() });
			    	} catch (NoSuchMethodException e3) {
				    	try {
				    		dispatch.method = getClass().getDeclaredMethod(symbol, new Class[] { args.getClass() });
	    ...
	    if ( dispatch.method != null )
	    	dispatch.method.setAccessible(true);
	    _inlet_dispatch.put(signature, dispatch);
	    
	    // reprocess now mapped message
	    _processInlet(symbol, args);
	}
